...
- **OAuth/OIDC: Google/Apple are cloud-only; generic OIDC works self-hosted.** `getOAuthProviders()` (`server/src/lib/oauth.ts`) gates Google/Apple behind `!config.selfHosted`, but the generic OIDC slot (`OIDC_ISSUER_URL`/`OIDC_CLIENT_ID`/`OIDC_CLIENT_SECRET`) is available in both modes — it's the one self-hosted admins configure against Authelia/Keycloak/etc. Discovery-driven (fetches `{issuer}/.well-known/openid-configuration`); `OIDC_ISSUER_URL` must be `https://`. `REQUIRE_OIDC_LOGIN=true` (`server/src/lib/passwordAuthGate.ts`) disables all password auth instance-wide — **before setting it, the admin must already have an OIDC identity linked to their account via Settings → Account → Connected Accounts**, since auto-link-by-email was removed for security (account takeover) and there's no admin CLI to link after the fact. Skipping this locks the admin out with no recovery except unsetting the env var and restarting.
